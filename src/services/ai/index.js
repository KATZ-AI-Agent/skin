...
class AIService {
  ...
  async processCommand(text, providedIntent, userId) {
    return circuitBreakers.executeWithBreaker(
      'openai',
      async () => {
        try {
          ...
          if (!intent) {
            // Try to match intent from text
            intent = matchIntent(text);
            
            // Default to CHAT if no specific intent matched
            if (!intent) {
          ...
          const result = await this.executeIntent(intent, userId, text, context);
  ...
  async executeIntent(intent, userId, text, context) {
    try {
      const network = await networkState.getCurrentNetwork(userId);

      switch (intent) {
        // Geeting and Chat intents, default
          case TRADING_INTENTS.CHAT:
          case TRADING_INTENTS.GREETING:
            const prompt = this.buildChatPrompt(text, context);
            const aiResponse = await openAIService.generateAIResponse(prompt, 'chat');
